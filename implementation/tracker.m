...
%+ Set parameters for keyfilter penalization coefficient
nu = params.nu;
...
while true
    ...
    %%                   
            ...
            % find maximum peak of the response map
            [disp_row, disp_col, sind] = resp_newton(response, responsef_padded, newton_iterations, ky, kx, output_sz);
            
            %+ find optimal coefficient theta for keyfilter penalization
            if (mod(seq.frame,keystep) == 0 || seq.frame == 2)
                response_pre_shift = circshift(response, [-floor(disp_row) -floor(disp_col)]);
            else
                ...
                [ref_theta, occ] = updateRefTheta(response_diff, params.gamma, nu);
            end
    ...
    for k = 1: numel(xlf)
        ...
        betha = 20;
        mumax = 10000;
        if seq.frame > 2 && mod(seq.frame,keystep) ~= 0
            gamma = ref_theta;
        else
            gamma = params.gamma;
        end
        ...
        while (i <= params.admm_iter)
                ...
                %   solve for H
                h = (T/((mu*T)+ params.admm_iter+ 2*gamma))* (ifft2((mu*g_f) + l_f) )+ 2*gamma.*h_current_key{k}/T;
                h0{k} = h;
                ...
                %   solve for gamma
                %if seq.frame > 2 && i < params.admm_iter && mod(seq.frame,keystep) ~= 0
                %    for t = 1:size(h_f, 3)
                %        z = power(norm(h_f(:,:,t) - h_current_key{k}(:,:,t), 2), 2) / 2;
                %        gamma = ref_theta - z;
                %    end
                %end
